...
{$R *.dfm}
   ...
   function CheckValidFile (): boolean;
    ...
    t1,t2: integer;
     begin
       if FileExists('C:\Users\Asus\Desktop\MyCourseWork\1 semester\Documents\Блокнот\check_valid.txt') then
         begin
           AssignFile(temp_file,'C:\Users\Asus\Desktop\MyCourseWork\1 semester\Documents\Блокнот\check_valid.txt');
           reset(temp_file);
           reset(initial_file);
         ...
         begin
            ...
            while(j<=last) and(counter<N_REC)  do
              begin
                str:=chr(i)+chr(j);
                for k := 0 to 9999 do
                   begin                            //(a..d)+(a..z)+(0000..9999)   (lаst dv)
      ...
      while ((i<= length(name)) and (not BadSimbol)) do
        begin
          if not( (name[i] in ['A'..'Z']) or (name[i] in ['a'..'z']) or
             (name[i] = '_') or (name[i] in ['А'..'Я']) or (name[i] in ['0'..'9'])
             or (name[i] in ['а'..'я']) ) then
          BadSimbol:= true;
          inc(i);
        end;
  ...
  procedure TForm6.AssignDefoltButtonClick(Sender: TObject);
    begin
      ...
      if FileExists('C:\Users\Asus\Desktop\MyCourseWork\1 semester\Documents\Блокнот\initial_file.txt') then
        begin
          AssignFile(initial_file,'C:\Users\Asus\Desktop\MyCourseWork\1 semester\Documents\Блокнот\initial_file.txt');
